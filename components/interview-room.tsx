...
import { TOTAL_INTERVIEW_QUESTIONS } from '@/lib/interview/state-machine';
  ...
  return (
    <main className="min-h-screen bg-[#09142e] text-white">
      ...
      <div className="mx-auto max-w-[1500px] px-5 py-6 lg:px-8">
        <div className="mb-6 grid gap-4 lg:grid-cols-[1fr_auto] lg:items-center"><div className="flex items-center gap-2">{levels.map((level, index) => { const activeIndex = levels.indexOf(data.interview.currentLevel); return <div key={level} className="flex flex-1 items-center lg:flex-none"><span className={`flex h-8 items-center gap-2 rounded-full px-3 text-xs font-semibold ${index === activeIndex ? 'bg-[#e6bd72] text-[#15203b]' : index < activeIndex ? 'bg-emerald-400/12 text-emerald-200' : 'bg-white/5 text-white/35'}`}>{index < activeIndex ? <Check className="size-3.5" /> : <span>{index + 1}</span>}<span className="hidden sm:inline">{levelLabels[level]}</span></span>{index < levels.length - 1 && <span className={`mx-2 h-px w-5 ${index < activeIndex ? 'bg-emerald-300/50' : 'bg-white/10'}`} />}</div>; })}</div><div className="flex items-center gap-3 text-xs text-white/55"><span>Question {current.sequence} of {TOTAL_INTERVIEW_QUESTIONS} · {completedQuestions} answered</span><Progress value={Math.min(100, (current.sequence / TOTAL_INTERVIEW_QUESTIONS) * 100)} className="w-32 [&_[data-slot=progress-track]]:bg-white/10 [&_[data-slot=progress-indicator]]:bg-[#e6bd72]" /></div></div>
